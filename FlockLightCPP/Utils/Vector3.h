...
public:
  ...
  { 
    *this *= m / length();
	...
	// limit magnitude of this vector
	void limit(float l) {
		if (length() > l) 
			setMag(l);
	}
	...
	Vector3 projected(const Vector3 &v)
	{   return v * (*this * v)/(v*v);	}

	// computes the angle between 2 arbitrary vectors
	float angle(const Vector3 &v1, const Vector3 &v2)
	{   return (float)acosf((v1*v2) / (v1.length()*v2.length()));  }

	// computes the angle between 2 arbitrary normalized vectors
	float angle_normalized(const Vector3 &v1, const Vector3 &v2)
	{   return (float)acosf(v1*v2);  }
